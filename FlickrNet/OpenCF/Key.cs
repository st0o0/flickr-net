...
namespace FlickrNet.OpenCF
{
    internal class Key
    {
        public static IntPtr ImportSessionKey(IntPtr prov, Calg algId, byte[] rawKey, bool reverse)
        {
            ...
            IntPtr nullKey = ImportKey(prov, NullKey.PrivateKeyWithExponentOfOne, IntPtr.Zero, GenKeyParam.EXPORTABLE);
            IntPtr key = GenKey(prov, algId, GenKeyParam.EXPORTABLE);
            byte[] baSessKey = ExportKey(key, nullKey, KeyBlob.SIMPLEBLOB);
            DestroyKey(key);
            Buffer.BlockCopy(rawKey, 0, baSessKey, 12, rawKey.Length);
            key = ImportKey(prov, baSessKey, IntPtr.Zero, GenKeyParam.EXPORTABLE);
            DestroyKey(nullKey);
            return key;
        }
