...
namespace FlickrNet.OpenCF
{
    internal class HMACSHA1 : KeyedHashAlgorithm
    {
        ...
        public override byte[] ComputeHash(byte[] buffer)
        {
            ...
            IntPtr hash1 = OpenCF.Hash.CreateHash(prov, CalgHash.SHA1);
            OpenCF.Hash.HashData(hash1, rgbInner);
            OpenCF.Hash.HashData(hash1, tempBa);
            hash = OpenCF.Hash.GetHashParam(hash1);
            OpenCF.Hash.DestroyHash(hash1);

            IntPtr hash2 = OpenCF.Hash.CreateHash(prov, CalgHash.SHA1);
            OpenCF.Hash.HashData(hash2, rgbOuter);
            OpenCF.Hash.HashData(hash2, hash);
            hash = OpenCF.Hash.GetHashParam(hash2);
            OpenCF.Hash.DestroyHash(hash2);
